...
import { arrayToBinaryString, binaryStringToArray, decodeBase64 } from '../utils';
...
/**
 * Get modulus keys.
 * Can cache keys since the SRP_MODULUS_KEY is constant.
 * @return {Promise}
 */
export const getModulusKeys = () => openpgp.key.readArmored(SRP_MODULUS_KEY);
...
export const verifyAndGetModulus = async (modulus) => {
    const [publicKeys, modulusParsed] = await Promise.all([
        getModulusKeys(), //
        openpgp.cleartext.readArmored(modulus)
    ]);
