...
export default async function keyInfo(rawKey, email, expectEncrypted = true, date = serverTime()) {
    ...
    const obj = {
        ...
        fingerprints: [mainFingerprint, ...getSubkeysFingerprints(key)],
        ...
        expires: await key.getExpirationTime('encrypt_sign').catch(() => null),
        ...
        decrypted: key.isDecrypted(), // null if public key
    ...
    const encryptCheck = obj.encrypt
        ? openpgp.encrypt({ message: createMessage('test message'), publicKeys: key, date })
        : Promise.resolve();
    await encryptCheck;
