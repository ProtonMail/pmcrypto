import { enums, PrivateKey, readKey } from '../openpgp';
...
export default async function keyInfo(rawKey, email, expectEncrypted = true, date = serverTime()) {
    const keyType = rawKey instanceof Uint8Array ? 'binaryKey' : 'armoredKey';
    const key = await readKey({ [keyType]: rawKey });
    ...
    const encryptCheck = obj.encrypt
        ? encryptMessage({ textData: 'test message', encryptionKeys: key, date })
        : Promise.resolve();
    await encryptCheck;
