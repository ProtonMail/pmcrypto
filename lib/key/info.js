...
async function keyInfo(rawKey, email, expectEncrypted = true, date = serverTime()) {
    ...
    const obj = {
        ...
        expires: await keys[0].getExpirationTime().catch(() => null),
        encrypt: await packetInfo(await keys[0].getEncryptionKeyPacket(undefined, date), keys[0]),
        sign: await packetInfo(await keys[0].getSigningKeyPacket(undefined, date), keys[0]),
    ...
    const encryptCheck = obj.encrypt ? openpgp.encrypt({ data: 'test message', publicKeys: keys, date }) : Promise.resolve();
    await encryptCheck;
