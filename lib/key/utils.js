...
export async function getPreferredAlgorithm(keys, date = serverTime()) {
    return enums.read(enums.symmetric, await keyModule.getPreferredAlgo('symmetric', keys, date));
}
...
export async function getKey(rawKey = '') {
    const method = rawKey instanceof Uint8Array ? 'read' : 'readArmored'; // key.read or key.readArmored
    const key = await keyModule[method](rawKey);
    return key;
}

export async function getKeys(rawKeys = '') {
    const method = rawKeys instanceof Uint8Array ? 'readAll' : 'readAllArmored'; // key.readAll or key.readAllArmored
    const keys = await keyModule[method](rawKeys);
    return keys;
}
