...
export async function generateForwardingMaterial(originalKey, forwardingUserIds, subKeyId = null) {
    ...
    const forwardingKeyObject = await openpgp.reformatKey({ privateKey: forwardingKey, userIds: forwardingUserIds });
    // Convert primary key to dummy
    forwardingKeyObject.key.keyPacket.makeDummy();
    ...
    return { proxyFactor, finalRecipientKey: forwardingKeyObject };
}
