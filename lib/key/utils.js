...
import { keySize, getRandomBytes } from '../utils';
...
export function generateSessionKey(algorithm) {
    if (!keySize.hasOwnProperty(algorithm)) {
        throw new Error('Unsopported algorithm for generating session key');
    }
    return getRandomBytes(keySize[algorithm]);
}
...
export async function genPublicEphemeralKey({ Curve, Q, Fingerprint }) {
    ...
    const param = buildEcdhParam(enums.publicKey.ecdh, Fingerprint);

    const Z = await kdf(S, keySize[cipherAlgo], param);
...
export async function genPrivateEphemeralKey({ Curve, d, V, Fingerprint }) {
    ...
    return kdf(S, keySize[cipherAlgo], param);
}
