...
export async function generateSessionKey(algorithm) {
...
export async function genPublicEphemeralKey(options) {
    ...
    const cipher_algo = openpgp.enums.read(openpgp.enums.symmetric, curveObj.cipher);
    ...
    const Z = await openpgp.crypto.publicKey.elliptic.ecdh.kdf(
        curveObj.hash,
        S,
        openpgp.crypto.cipher[cipher_algo].keySize,
        param,
        curveObj,
        false
    );
