...
export async function getKeys(rawKeys = '') {
    const method = rawKeys instanceof Uint8Array ? 'read' : 'readArmored'; // openpgpjs.key.read or openpgpjs.key.readArmored
    const keys = await openpgpjs.key[method](rawKeys);
