...
export async function getKeys(rawKeys = '') {
    const keys = rawKeys instanceof Uint8Array ? await openpgpjs.key.read(rawKeys) : await openpgpjs.key.readArmored(rawKeys);

    if (!keys) { // keys is undefined in that case
...
export async function getMatchingKey(signature, keys) {
    ...
    const [key = null] = keyids.map((keyid) => keyring.getKeysForId(keyid, true) || []).flatten().filter(Boolean);
