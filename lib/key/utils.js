...
export async function getKey(rawKey = '') {
    const method = rawKey instanceof Uint8Array ? 'read' : 'readArmored'; // openpgp.key.read or openpgp.key.readArmored
    const key = await openpgp.key[method](rawKey);
    return key;
}

export async function getKeys(rawKeys = '') {
    const method = rawKeys instanceof Uint8Array ? 'readAll' : 'readAllArmored'; // openpgp.key.readAll or openpgp.key.readAllArmored
