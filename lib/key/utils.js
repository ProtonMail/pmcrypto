...
export async function genPublicEphemeralKey({ Curve, Q, Fingerprint }) {
    ...
    const Z = await kdf(curveObj.hash, S, crypto.cipher[cipherAlgo].keySize, param, false, false);
...
export async function genPrivateEphemeralKey({ Curve, d, V, Fingerprint }) {
    ...
    return kdf(curveObj.hash, S, crypto.cipher[cipherAlgo].keySize, param, false, false);
}
