...
// ECDHE
export const genPublicEphemeralKey = (curve, Q, fingerprint) => {
    const infos = openpgp.crypto.public_key.elliptic.curves.curves[curve];
    openpgp.crypto.public_key.elliptic.ecdh.genPublicEphemeralKey(infos["oid"], info["cipher_algo"], info["hash_algo"],
        Q, fingerprint);
};

export const genPrivateEphemeralKey = (curve, V, d, fingerprint) => {
    const infos = openpgp.crypto.public_key.elliptic.curves.curves[curve];
    openpgp.crypto.public_key.elliptic.ecdh.genPrivateEphemeralKey(infos["oid"], info["cipher_algo"], info["hash_algo"],
        V, d, fingerprint);
};
