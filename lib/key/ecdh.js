...
import { concatArrays, binaryStringToArray, getRandomBytes, digest } from '../utils';
...
export async function kdf(hashAlgo, X, length, param, stripLeading = false, stripTrailing = false) {
    ...
    const digestResult = await digest(hashAlgo, concatArrays([new Uint8Array([0, 0, 0, 1]), X, param]));
