...
export function keyCheck(info, email, expectEncrypted) {
    ...
    if (info.version !== 4 && info.version !== 5) {
        throw new Error('Key is not OpenPGP version 4 or 5');
    }
    ...
    if (info.user.hash && info.user.hash.length) {
        if (info.user.hash[0] !== enums.hash.sha256) {
    ...
    if (info.user.symmetric && info.user.symmetric.length) {
        if (info.user.symmetric[0] !== enums.symmetric.aes256) {
    ...
    if (info.user.compression && info.user.compression.length) {
        if (info.user.compression[0] !== enums.compression.zlib) {
