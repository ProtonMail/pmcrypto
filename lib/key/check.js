...
export function keyCheck(info, email, expectEncrypted) {
    ...
    if (!['rsaEncryptSign', 'ecdsa', 'eddsa'].includes(info.algorithmName)) {
    ...
    if (info.user.hash && info.user.hash.length) {
        if (!info.user.hash.includes(enums.hash.sha256)) {
            throw new Error('Preferred hash algorithms must include SHA256');
        }
    } else {
    ...
    if (info.user.symmetric && info.user.symmetric.length) {
        if (!info.user.symmetric.includes(enums.symmetric.aes256)) {
            throw new Error('Preferred symmetric algorithms must include AES256');
        }
    } else {
    ...
    if (info.user.compression && info.user.compression.length) {
        if (!info.user.compression.includes(enums.compression.zlib)) {
            throw new Error('Preferred compression algorithms must include zlib');
        }
    }
