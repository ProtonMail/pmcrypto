 ...
 * @async
 */
export async function checkContactKeyStrength(publicKey) {
    const { enums } = openpgp;
    ...
    // primary key only (TODO does this matter for contacts?)
    checkKeyStrength(publicKey, {
        allowedPublicKeyAlgorithms: new Set([
            enums.publicKey.rsa_encrypt_sign,
            enums.publicKey.ecdsa,
    ...
    const encryptionSubkey = await publicKey.getEncryptionKey().catch(() => null);
