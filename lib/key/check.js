...
export function dateChecks([ key ], date = serverTime()) {
    ...
    if (!keys.every(({ keyPacket }) => keyPacket.created < date)) {
        throw new Error('The sub key key packets are created with illegal times');
    }
    if (!key.users.every(({ selfCertifications }) => selfCertifications.every(({ created }) => created < date))) {
        throw new Error('The self certifications are created with illegal times');
    }
    if (!keys.every(({ bindingSignatures = [] }) => bindingSignatures.every(({ created }) => created < date))) {
