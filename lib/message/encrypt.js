...
export default async function encryptMessage({
    ...
    if (detached) {
        ...
        const { message } = sanitizedOptions; // TODO clone input stream -- sanitizedOptions.message.fromStream ? await readMessage({ clone(<input data>) }) : sanitizedOptions.message;
