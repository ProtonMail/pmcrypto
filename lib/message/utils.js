...
export function createMessage(source, filename, date = serverTime()) {
    if (Uint8Array.prototype.isPrototypeOf(source)) {
        return openpgpCreateMessage({ binary: source, filename, date });
    }
    return openpgpCreateMessage({ text: source, filename, date });
}

export async function signMessage({ armor = true, ...options }) {
    if (typeof options.data === 'string') {
        options.message = await createCleartextMessage(options.data);
    }

    if (Uint8Array.prototype.isPrototypeOf(options.data)) {
        options.message = await createMessage(options.data);
    }
