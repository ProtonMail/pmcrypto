...
export async function verifyMessage(options) {
    const { publicKeys = [], date: optionDate } = options;
    const date = typeof optionDate === 'undefined' ? serverTime() : optionDate;
    const result = await openpgp.verify({ ...options, date });
    return handleVerificationResult(result, publicKeys, date);
}
