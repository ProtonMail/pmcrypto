...
export function createMessage(source, filename) {
...
export async function handleVerificationResult({ data, filename = 'msg.txt', signatures: sigs }, publicKeys, date) {
    ...
    if (verified === SIGNED_AND_INVALID) {
        ...
        return verificationResults.filter(({ verified }) => verified === SIGNED_AND_VALID).reduceRight(
            (acc, result) => {
...
export async function splitMessage(message) {
    ...
    const keyFilter = (packet) => {
        return (
            packet.tag !== openpgpjs.enums.packet.publicKeyEncryptedSessionKey &&
