...
export async function decryptMessage(options) {
    ...
    try {
        ...
        const { data, filename, verified, errors } = await handleVerificationResult(result, publicKeys, options.date);
...
export async function decryptMIMEMessage(options) {
    const { data: rawData, verified: embeddedVerified, errors, signatures: pgpSignatures } = await decryptMessageLegacy(
        options
    );
    ...
    return {
        ...
        stop() {},
        signatures: [...pgpSignatures, ...mimeSignatures]
    };
}
