...
import { decodeUtf8Base64, binaryStringToArray, arrayToBinaryString, isStream, transform } from '../utils';
...
export async function decryptMessageLegacy(options) {
    ...
    if (isStream(oldEncMessage.subarray(blockSize + 2))) {
        const cfb = new AES_CFB(randomKey, oldEncMessage.subarray(2, blockSize + 2));
        return transform(
            oldEncMessage.subarray(blockSize + 2),
    ...
    const result = AES_CFB.decrypt(
        oldEncMessage.subarray(blockSize + 2),
        randomKey,
        oldEncMessage.subarray(2, blockSize + 2)
    );
