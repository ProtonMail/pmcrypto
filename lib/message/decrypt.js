...
function decryptMessage(options) {
    ...
    return Promise.resolve()
        .then(() => {
            ...
            try {
                return openpgp.decrypt(optionsPrivate)
                    .then(({ data, filename, signatures: sigs }) => {
                        ...
                        if (sigs && sigs.length) {
                            ...
                            for (let i = 0; i < sigs.length; i++) {
                                ...
                                if (sigs[i].valid || (!optionsPrivate.publicKeys || !optionsPrivate.publicKeys.length)) {
                        ...
                        if (process.env.NODE_ENV !== 'production') {
                            switch (verified) {
                                ...
                                case SIGNED_AND_INVALID:
                                    ...
                                    break;
                                default:
                                    return Promise.reject(new Error('Unknown verified value'));
                            }
                        }
