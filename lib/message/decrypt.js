import { isStream } from '@openpgp/web-stream-tools';
...
export async function decryptMessage({ encryptedSignature, armor = true, ...options }) {
    const { publicKeys = [] } = options;
    options.date = typeof options.date === 'undefined' ? serverTime() : options.date;

    const format = armor ? 'armored' : 'object';
    options.format = typeof options.format === 'undefined' ? format : options.format;
    ...
    try {
        ...
        if (encryptedSignature) {
            const decryptedSignature = await decrypt({
                ...options,
                message: await getMessage(encryptedSignature),
                format: 'binary'
            });
            options.signature = await readSignature({ binarySignature: decryptedSignature.data });
        }
...
export async function decryptMessageLegacy(options) {
    ...
    const result = await AES_CFB.decrypt(oldEncMessage.subarray(16 + 2), randomKey, oldEncMessage.subarray(2, 16 + 2));
