import { decrypt, crypto, readSignature } from 'openpgp';
import { decodeUtf8Base64, binaryStringToArray, arrayToBinaryString, isStream } from '../utils';
...
export async function decryptMessageLegacy(options) {
    ...
    const result = await crypto.cfb.decrypt(
        'aes256',
        randomKey,
        oldEncMessage.subarray(crypto.cipher.aes256.blockSize + 2),
        oldEncMessage.subarray(2, crypto.cipher.aes256.blockSize + 2)
    );
