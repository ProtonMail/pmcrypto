...
export async function decryptMessage({ date = serverTime(), encryptedSignature, armor = true, ...options }) {
    ...
    try {
        ...
        if (encryptedSignature) {
            ...
            sanitizedOptions.signature = await readSignature({ binarySignature: decryptedSignature });
        }

        const result = await decrypt(sanitizedOptions);
        const verificationResult = handleVerificationResult(result);
