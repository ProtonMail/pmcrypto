...
import { decodeUtf8Base64, binaryStringToArray, arrayToBinaryString, isStream, cipher, transform } from '../utils';
...
export async function decryptMessage(options) {
    ...
    try {
        ...
        if (options.encryptedSignature) {
            ...
            options.signature = await readSignature(decryptedSignature.data);
        }

        const { data, signatures, filename } = await decrypt(options);
        const verificationResult = handleVerificationResult({ data, signatures }, publicKeys, options.date);
...
export async function decryptMessageLegacy(options) {
    ...
    if (isStream(oldEncMessage.subarray(cipher.aes256.blockSize + 2))) {
        const cfb = new AES_CFB(randomKey, oldEncMessage.subarray(2, cipher.aes256.blockSize + 2));
        return transform(
            oldEncMessage.subarray(cipher.aes256.blockSize + 2),
            (value) => cfb.AES_Decrypt_process(value),
            () => cfb.AES_Decrypt_finish()
        );
    }
    const result = AES_CFB.decrypt(
        oldEncMessage.subarray(cipher.aes256.blockSize + 2),
        randomKey,
        oldEncMessage.subarray(2, cipher.aes256.blockSize + 2)
    );
