...
const verifySignature = async ({ publicKeys, date }, data) => {
    ...
    const { data: subdata, verified } = await verifyMessage({
        message: getCleartextMessage(body), publicKeys, date, signature
    });

    return { subdata, verified };
};
 ...
 * @returns {Promise.<*>}
 */
const parse = async ({ headerFilename = 'Encrypted Headers.txt', sender = null }, content = '', verified = VERIFICATION_STATUS.NOT_VERIFIED) => {
    const data = await parseMail(content);
    // cf. https://github.com/autocrypt/memoryhole subject can be in the MIME headers
    const {
        attachments = [], text = '', html = '', subject: mimeSubject = false
    } = data;
    ...
    if (html) {
        return {
            body: html, attachments, verified, encryptedSubject, mimetype: 'text/html'
        };
    }
    if (text) {
        return {
            body: text, attachments, verified, encryptedSubject, mimetype: 'text/plain'
        };
    }
    if (attachments.length) {
        return {
            attachments, verified, encryptedSubject
        };
    }
...
export default async function processMIME(options, data) {
    const { subdata, verified } = await verifySignature(options, data);

    return parse(options, subdata, verified);
}
