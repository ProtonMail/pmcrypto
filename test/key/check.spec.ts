...
export const experimentalPQCKeyV6 = `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
describe('key checks', () => {
    ...
    it('checkKeyStrength - it does not warn on secure key (experimental PQC)', async () => {
        const pqcKey = await readKey({ armoredKey: experimentalPQCKeyV6 });
        expect(
            () => checkKeyStrength(pqcKey)
        ).to.not.throw();
    });
...
-----END PGP PRIVATE KEY BLOCK-----`
    ...
    it('checkKeyCompatibility - it rejects a v6 PQC key unless explicitly allowed', async () => {
        const key = await readKey({ armoredKey: experimentalPQCKeyV6 });
        expect(
            () => checkKeyCompatibility(key)
        ).to.throw(/Version 6 keys are currently not supported/);
        expect(
